...
data_file = f"{root}results/tmax_cluster_data_{month_string}.nc"
# %% Load data
with Dataset(data_file,'r') as nc:
  ...
  tmax = nc.variables['tmax_extremes'][:]
  clusters = nc.variables['clusters_extremes'][:]
  tmax_mean = np.zeros((Nclusters,Nlat,Nlon))
  for ic in range(Nclusters):
    tmax_mean[ic,:,:] = np.sum(tmax*(clusters-1==ic),axis=0)/counts[ic,:,:][None,:,:]
  tmax_mean = np.where(counts>0,tmax_mean,np.nan)
...
shading *= mask
# %% Plot results
pdf = PdfPages(f'{path_out}tmax_clusters_{month_string}.pdf')
...
ax.set_title(f'Dominant SWT for 5% warmest days in {month_string}',fontsize=14)
pdf.savefig()
# %% Probabilities
...
# %% Occurance
fig = plt.figure(figsize=(9,9))
fig.suptitle(f"Significant SWT occurance in 5% warmest days in {month_string}",y=0.93,fontsize=14)
...
for irow in range(6):
    for icol in range(5):
        ...
        cf=ax.contourf(lon2d,lat2d,np.where((signif[iplot,:,:] & mask),counts[iplot,:,:]/k*100,np.nan),levels=np.linspace(0,35,8),cmap="inferno",extend='max')
...
# %% Temperature
fig = plt.figure(figsize=(9,9))
fig.suptitle(f"Mean temperature for 5% warmest days in {month_string}",y=0.93,fontsize=14)
gs = GridSpec(6, 5+1, width_ratios=[4]*5+[1],hspace=0)
i = 0
for irow in range(6):
    for icol in range(5):
        ...
        cf=ax.contourf(lon2d,lat2d,np.where((signif[iplot,:,:] & mask),tmax_mean[iplot,:,:],np.nan),levels=np.linspace(30,45,11),cmap="inferno",extend='both')
        ax.set_title(SWT[iplot])
        ax.add_feature(LAND,facecolor='lightgrey')
        ax.coastlines(linewidths=0.4)
        ax.set_xlim([lon.min(),lon.max()])
...
ax_cbar = fig.add_subplot(gs[0:2,-1])
cax = inset_axes(ax_cbar, width="100%", height="80%", loc="center") 
plt.colorbar(cf,cax=cax,label=f"Degrees Celsius")
ax_cbar.axis("off")
pdf.savefig()
pdf.close()
# %%
